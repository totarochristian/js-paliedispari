/*
---------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------   Main Goal   ------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------
Palidrome
Ask the user to enter a word
Create a function to understand if the entered word is a palindrome

Even and odd
The user chooses even or odd and enters a number from 1 to 5.
We generate a random number (always from 1 to 5) for the computer (using a function).
Let's add the two numbers
We determine if the sum of the two numbers is even or odd (using a function)
We declare who won.
---------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------   Tips   --------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------
Tips of the day
1. We always write the steps we want to do in Italian
2. We always write only one piece of code at a time, if it works then let's move on.
---------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------
*/

// Event listeners for the "Palidrome" goal
document.getElementById("wordSubmit").addEventListener("click",CheckPalidrome);
document.getElementById("word").addEventListener("change",CheckPalidrome);

// Event listeners for the "Even and odd" goal
document.getElementById("evenOddSubmit").addEventListener("click",CheckEvenOdd);
document.getElementById("evenOddSelect").addEventListener("change",CheckEvenOdd);
document.getElementById("number").addEventListener("change",CheckEvenOdd);